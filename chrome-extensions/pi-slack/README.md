...
The Chrome extension needs the shared secret generated by the Pi side to authenticate.
See `manual-extensions/pi-slack/README.md` — **First-run setup** — for the full three-step
flow (launch `pi-slack`, run `/slack-status --show-token`, paste into this popup).
...
5. Open the extension popup, paste the shared-secret token from the Pi side, and press **Save**.
